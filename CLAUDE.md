...
- **`TerrainRenderer`** (`src/core/terrain_renderer.*`) — a `RefCounted` that owns all `RenderingServer` RIDs directly (mesh, ring mesh, shader, and one instance+material per clipmap level) rather than going through Godot scene nodes. Builds the clipmap ring hierarchy in `rebuild_mesh()` and re-snaps/re-scales each level around the current focus point in `update_focus_position()`. Every uniform and instance bound derived from the configuration is pushed by one shared `_push_parameters()` — called by `rebuild_mesh()` once the levels exist, and again by `refresh_parameters()` whenever a caller wants a configuration edit on screen without rebuilding geometry or re-uploading textures (animating a value per frame; `Terrain3D::refresh_parameters()`). Put anything configuration-derived there, not in `rebuild_mesh()`, or the live path silently drifts from the rebuild path. `set_shader_parameter()` sets a uniform outside the configuration for tooling (the debug views, the trailer's diagram overlay), remembering it so a later rebuild re-applies it.
...
The shader's last section is a **diagram overlay block** for tooling only — the `debug_view` visualisations, plus a second view with a world-space wipe between the two, a wireframe drawn off the mesh's own `UV * resolution` cell grid, a Chebyshev reveal radius, an emission gain and a vignette. Every uniform there is neutral at its default and the whole block sits behind one uniform-coherent branch at the end of `fragment()`, so a normal render is byte-identical to one with the block absent; keep it that way when editing it. Nothing in the plugin sets these: they are reached through `Terrain3D::set_shader_parameter()`, which is how `demo/trailer/` drives them.
