...
void TerrainRenderer::rebuild_mesh(float p_size, int p_resolution) {
    ...
    rs->shader_set_code(_internal_shader_rid, R"(
        ...
        void vertex() {
            ...
            vec2 e = vec2(1.0 / tex_size.x, 1.0 / tex_size.y); // Tamaño de un píxel

            float h_l = texture(height_map, UV - vec2(e.x, 0.0)).r;
            float h_r = texture(height_map, UV + vec2(e.x, 0.0)).r;
            float h_u = texture(height_map, UV - vec2(0.0, e.y)).r;
            float h_d = texture(height_map, UV + vec2(0.0, e.y)).r;
...
void TerrainRenderer::set_user_material(const Ref<Material> &p_material) {
	if (!_internal_material_rid.is_valid()) return;
